  ...
  return (
    ...
    >
      ...
      <div className="relative h-56 overflow-hidden img-hover">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4">
          <div className="flex items-end justify-between">
            <div>
              <p className="text-white/85 text-xs tracking-[1.5px] uppercase mb-1">{pkg.type}</p>
      ...
      <div className="p-5" style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
        <div className="flex items-center gap-5 text-sm mb-4" style={{ color: '#374151' }}>
          <span className="flex items-center gap-1.5">
            <FiMapPin size={12} style={{ color: '#b8975a' }} />{pkg.destination}
          </span>
          <span className="flex items-center gap-1.5">
            <FiClock size={12} style={{ color: '#b8975a' }} />{pkg.duration}
          </span>
          <span className="flex items-center gap-1.5">
            <FiUsers size={12} style={{ color: '#b8975a' }} />{pkg.seats} seats
          </span>
        </div>
        ...
        <div className="flex items-center justify-between pt-4" style={{ borderTop: '1px solid #f0ede6', marginTop: 'auto' }}>
          <div>
            <p className="text-xs uppercase tracking-wider mb-0.5 font-medium" style={{ color: '#4b5563' }}>From</p>
            ...
            <p className="text-xs" style={{ color: '#4b5563' }}>per person</p>
          </div>
          <Link
            to={`/packages/${pkg.id}`}
            className="flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-bold uppercase tracking-wider transition-all"
            style={{ background: '#0a192f', color: '#fff', letterSpacing: '0.8px' }}
            onMouseEnter={e => { e.currentTarget.style.background = '#b8975a'; }}
            onMouseLeave={e => { e.currentTarget.style.background = '#0a192f'; }}
            aria-label={`View details for ${pkg.title}`}
          >
            View <HiArrowLongRight size={16} />
          </Link>
        </div>
      </div>
    </motion.article>
