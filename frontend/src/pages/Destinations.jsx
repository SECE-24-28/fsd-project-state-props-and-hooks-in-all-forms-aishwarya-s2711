        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-center mb-8">
            ...
            <div className="relative flex-1 min-w-48 rounded-xl border border-gray-200 px-4 py-2.5 flex items-center" style={{ background: '#fff' }}>
              <FiSearch size={14} style={{ color: '#b8975a' }} />
              <input
                type="text"
              ...
              {CATEGORIES.map(cat => (
                <li key={cat}>
                  <button
                    ...
                    style={category === cat ? { background: '#0a192f', color: '#fff' } : { background: 'transparent', color: 'var(--text-mid)', border: '1px solid #e8e4dc' }}
                  >
                    {cat}
                  </button>
                </li>
          ...
          {filtered.length > 0 ? (
            ...
            <div className="text-center py-20" style={{ background: '#fff' }}>
              <p className="text-2xl font-light" style={{ color: '#0a192f' }}>No destinations match your criteria.</p>
        ...
        <div className="fixed inset-0 z-50 lg:hidden" aria-modal="true" role="dialog">
          ...
          <div className="absolute top-0 left-0 bottom-0 w-80 bg-white p-6 overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-semibold" style={{ color: '#0a192f' }}>Filters</h3>
              ...
              {CATEGORIES.map(cat => (
                <li key={cat}>
                  <button
                    ...
                    style={category === cat ? { background: '#0a192f', color: '#fff' } : { background: '#f5f5f5', color: 'var(--text-dark)' }}
                  >
                    {cat}
                  </button>
                </li>
