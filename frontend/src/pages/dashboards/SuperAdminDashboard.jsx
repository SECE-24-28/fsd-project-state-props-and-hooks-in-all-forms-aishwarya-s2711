...
function Overview() {
  const [data] = useState(() => ({
    bookings: adminState.getBookings(),
    users: adminState.getUsers(),
    packages: adminState.getPackages(),
    payments: adminState.getPayments()
  }));
...
function BookingsManagement() {
  const [bookings, setBookings] = useState(() => adminState.getBookings());
  ...
  const exportReport = () => {
    ...
    const rows = bookings.map(b => [b.id, b.user, b.tour, b.date, b.persons, b.amount, b.status]);
  ...
  const filtered = bookings.filter(b => {
    const matchesSearch = b.user.toLowerCase().includes(searchTerm.toLowerCase()) || b.tour.toLowerCase().includes(searchTerm.toLowerCase()) || b.id.toLowerCase().includes(searchTerm.toLowerCase());
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {filtered.map(b => (
                <tr key={b.id} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-6 py-4 text-xs font-mono font-bold text-slate-400">{b.id}</td>
                  <td className="px-6 py-4 text-sm font-semibold text-slate-900">{b.user}</td>
                  <td className="px-6 py-4 text-sm text-slate-600">{b.tour}</td>
