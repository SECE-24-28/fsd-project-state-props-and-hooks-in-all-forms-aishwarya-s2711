...
function FAQItem({ item, index }) {
  ...
  return (
    <div className={`faq-item ${open ? 'open' : ''}`}>
      ...
      >
        <span className="text-base font-semibold pr-4" style={{ fontFamily: 'DM Sans, sans-serif', color: '#0a192f' }}>
          {item.q}
        </span>
        <span className="shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-all"
          style={{ background: open ? '#0a192f' : 'rgba(184,151,90,0.10)', color: open ? '#b8975a' : '#0a192f' }}>
        ...
        {open && (
          ...
          >
            <div className="px-6 pb-6">
              <div className="h-px mb-4" style={{ background: 'rgba(184,151,90,0.20)' }} />
              <p className="text-sm leading-relaxed" style={{ color: '#4a4a4a' }}>{item.a}</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="min-h-screen page-enter" style={{ background: '#ffffff' }}>
        ...
        <section className="py-20" style={{ background: '#ffffff' }} aria-label="FAQ content">
          <div className="max-w-4xl mx-auto px-6">
            ...
            <div className="flex flex-wrap gap-2 mb-12 justify-center">
              {categories.map(c => (
                <button
                  ...
                  style={{
                    background: activeCategory === c ? '#0a192f' : 'transparent',
                    color: activeCategory === c ? '#fff' : '#4a4a4a',
                    border: `1.5px solid ${activeCategory === c ? '#0a192f' : '#e8e4dc'}`,
                    letterSpacing: '1.5px',
                  }}
                >
                  {c}
              ...
              {filtered.map((group, gi) => (
                ...
                >
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <h2 className="text-xl font-semibold" style={{ fontFamily: 'Cormorant Garamond, serif', color: '#0a192f' }}>
                      {group.category}
                    </h2>
                  </div>
            ...
            >
              <p className="label-text justify-center mb-4" style={{ color: '#b8975a' }}>Still Have Questions?</p>
              <h3 className="text-4xl font-light text-white mb-4" style={{ fontFamily: 'Cormorant Garamond, serif' }}>
                We're Here to Help
              </h3>
              <p className="mb-8" style={{ color: 'rgba(255,255,255,0.65)', fontSize: '0.95rem' }}>
