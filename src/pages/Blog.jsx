              ...
              <Link to={`/blog/${filtered[0].id}`} className="block">
                <div className="relative h-80 lg:h-[420px] overflow-hidden">
                  ...
                  <div className="absolute bottom-0 left-0 p-8 lg:p-12 max-w-2xl">
                    ...
                    <div className="flex items-center gap-4 text-xs" style={{ color: 'rgba(255,255,255,0.60)' }}>
            ...
            {(category === 'All' && !search ? filtered.slice(1) : filtered).map((blog, i) => (
              ...
              >
                ...
                <div className="p-8" style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
                  <div className="flex items-center gap-3 text-xs mb-4" style={{ color: '#666' }}>
                  ...
                  <p className="text-sm mb-5" style={{ color: '#333', lineHeight: 1.5, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{blog.excerpt}</p>
          ...
          <article className="flex-1 min-w-0">
            <div className="rounded-3xl p-8 lg:p-12 mb-8" style={{ background: '#fff', boxShadow: '0 8px 32px rgba(10,25,47,0.07)' }}>
              <p className="text-lg leading-relaxed mb-8 italic"
                style={{ color: '#4a4a4a', borderLeft: '3px solid #b8975a', paddingLeft: 20, fontFamily: 'Cormorant Garamond, serif', fontSize: '1.2rem' }}>
                {blog.excerpt}
              </p>
                    ...
                    if (isHeading) {
                      ...
                      return (
                        <div key={i}>
                          ...
                          {body && <p className="text-sm leading-8" style={{ color: '#4a4a4a' }}>{body}</p>}
                        </div>
                      );
                    }
                  }
                  return (
                    <p key={i} className="text-sm leading-8" style={{ color: '#4a4a4a' }}>{para}</p>
                  );
                })}
              </div>
          ...
          <aside className="hidden lg:block w-72 shrink-0">
            <div className="sticky top-28 space-y-6">
              ...
              <div className="rounded-2xl p-6" style={{ background: '#fff', boxShadow: '0 4px 24px rgba(10,25,47,0.06)' }}>
                ...
                <p className="text-xs leading-6" style={{ color: '#888' }}>
                ...
                <div className="space-y-4">
                  {blogs.filter(b => b.id !== blog.id).slice(0, 4).map(b => (
                    <Link key={b.id} to={`/blog/${b.id}`} className="flex gap-3 group">
                      ...
                      <div>
                        <p className="text-xs font-medium line-clamp-2 group-hover:text-[#b8975a] transition-colors"
                          style={{ color: '#0a192f' }}>{b.title}</p>
                        <p className="text-xs mt-1" style={{ color: '#aaa' }}>{b.readTime}</p>
                      </div>
                    </Link>
                  ))}
                </div>
