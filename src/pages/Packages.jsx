...
function FilterPanel({ type, setType, budget, setBudget, activeFilters, clearFilters }) {
  return (
    <div>
      ...
      <div className="mb-6">
        <p className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: '#888' }}>Tour Type</p>
        <div className="space-y-2">
          {TYPES.map(t => (
            <label key={t} className="flex items-center gap-3 cursor-pointer">
              <input type="radio" name="type" checked={type === t} onChange={() => setType(t)}
                style={{ accentColor: '#b8975a' }} />
              <span className="text-sm" style={{ color: type === t ? '#0a192f' : '#888', fontWeight: type === t ? 600 : 400 }}>
                {t}
              </span>
            </label>
          ))}
      ...
      <div className="mb-6">
        <p className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: '#888' }}>Budget</p>
        <div className="space-y-2">
          {BUDGETS.map((b, i) => (
            <label key={i} className="flex items-center gap-3 cursor-pointer">
              <input type="radio" name="budget" checked={budget === i} onChange={() => setBudget(i)}
                style={{ accentColor: '#b8975a' }} />
              <span className="text-sm" style={{ color: budget === i ? '#0a192f' : '#888', fontWeight: budget === i ? 600 : 400 }}>
                {b.label}
              </span>
            </label>
          ))}
